            ...
            case 'Array': {
                ...
                if (!isExpectedType) {
                    if (!source || typeof source[key] === 'undefined') {
                        errors.stack.push(`<${key}> property is missing`);
                    }
                    else {
                        const errorKey = Number.isInteger(+key) ? `${errors.key}[${key}]`: key;
                        errors.stack.push(`<${errorKey}> property expected to be a ${valueTypeConstructor.name} but it was ${source[key].constructor.name}`);
                    }
                }
                break;
            }
